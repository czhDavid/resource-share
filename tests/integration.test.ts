...
describe('CLI: status', () => {
  ...
  it('should return unlocked status for a resource that was never locked', async () => {
    const { json, exitCode } = await runCliJson(['status', 'unknown-resource'], lockDir);
  ...
  it('should return status for multiple acquired resources (stale cleanup)', async () => {
    ...
    await runCliJson(['acquire', 'res-a', '--holder', 'agent-1', '--reason', 'first'], lockDir);
    await runCliJson(['acquire', 'res-b', '--holder', 'agent-2', '--reason', 'second'], lockDir);
...
describe('CLI: clear', () => {
  ...
  it('should clear all locks with --force', async () => {
    await runCliJson(['acquire', 'res-a', '--holder', 'agent-1', '--reason', 'a'], lockDir);
    await runCliJson(['acquire', 'res-b', '--holder', 'agent-2', '--reason', 'b'], lockDir);
...
describe('CLI: full workflow (engine + CLI combined)', () => {
  ...
  it('should complete acquire → status → release → status cycle', async () => {
    ...
    const { json: statusJson } = await runCliJson(['status', 'app-server'], lockDir);
    ...
    const { json: statusJson2 } = await runCliJson(['status', 'app-server'], lockDir);
