...
try {
  ...
  log("INFO", `Loaded ${Object.keys(resourceConfig.resources).length} resource(s) from ${configPath}`);
} catch (err) {
...
server.registerTool(
  ...
  async ({ resource, holder, reason, ttl }) => {
    log("INFO", `tool=acquire resource=${resource} holder=${holder} reason="${reason ?? ""}" ttl=${ttl ?? "default"}`);
    const err = validateResource(resource);
    if (err) {
      log("ERROR", `tool=acquire validation failed: ${err}`);
      return { content: [{ type: "text" as const, text: JSON.stringify({ ok: false, error: err }) }] };
    }
...
server.registerTool(
  "agent_lock_clear",
  {
    ...
    inputSchema: {
      force: z.boolean().optional().describe("Remove all locks, not just stale ones (default: false)"),
    },
  },
  async ({ force }) => {
